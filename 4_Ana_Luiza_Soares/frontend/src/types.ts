...
export type WeeklyPriorityResult = { priority_item_id: string; discipline_id: string; discipline_code?: string | null; discipline_name: string; content_id?: string | null; assessment_id?: string | null; assessment_name?: string | null; deadline_at?: string | null; priority_score: number; priority_band: "low" | "medium" | "high"; evidence_used: { source_type: string; source_id?: string | null; summary: string }[]; missing_evidence: string[]; reason: string; estimated_demand_minutes?: number | null; demand_confidence: number; demand_reason: string };
...
export type PriorityCapacityAnalysis = { priority_item_id: string; discipline_id: string; discipline_name: string; requested_minutes?: number | null; allocated_minutes: number; remaining_minutes?: number | null; available_minutes_before_deadline: number; usable_minutes_before_deadline: number; blocked_minutes: number; allocated_to_higher_priorities_minutes: number; minimum_useful_block_minutes: number; deadline_at?: string | null; reason_code: "fully_allocated" | "partially_allocated" | "insufficient_conflict_free_capacity" | "no_window_before_deadline" | "fragments_below_minimum" | "deadline_expired" | "demand_unknown" | "excluded_by_user" | "conflict_introduced_after_preview"; reason: string; blocking_events: { event_id: string; title: string; blocked_minutes: number }[] };
export type WeeklyPlanPreview = { study_plan_id: string; week_start: string; timezone: "America/Sao_Paulo"; availability: AvailabilitySummary; ranked_priorities: WeeklyPriorityResult[]; planned_blocks: PlannedStudyBlockPreview[]; unallocated_priorities: WeeklyPriorityResult[]; capacity_analysis: PriorityCapacityAnalysis[]; conflicts: string[]; warnings: string[]; algorithm_version: string; generated_at: string };
