...
type WeightFields = { weight?: number | null; group_final_weight?: number | null; group_weight?: number | null };
function weightSummary(item: WeightFields) {
  if (item.group_final_weight != null && item.group_weight != null) return `Grupo vale ${numberText(item.group_final_weight, "%")} da disciplina · ${numberText(item.group_weight, "%")} dentro do grupo · peso efetivo: ${numberText(effectiveWeight(item), "%")}`;
  return `Peso efetivo: ${numberText(effectiveWeight(item), "%")}`;
}

function effectiveWeight(item: WeightFields) {
...
function coursePlanPreviewState(preview: CoursePlanPreviewResponse | null, data: CoursePlanData | null) {
  if (!preview || !data) return null;
  if (preview.source === "local_parser" && preview.fallback_reason) return "Extração inteligente indisponível; parser local usado.";
  if (data.assessments.length === 0) return "Nenhuma avaliação foi identificada automaticamente.";
  if (preview.warnings.length > 0) return "Extração parcial disponível para revisão.";
  return "Extração concluída. Revise antes de confirmar.";
}
...
export function DisciplineDetailPage({ disciplineId, onBack }: Props) {
  ...
  async function handlePlanPreview(file?: File) {
    ...
    try {
      ...
      setNotice(coursePlanPreviewState(response, response.data) ?? "Pré-visualização gerada. Revise antes de confirmar.");
  ...
  return (
    <div className="page detail-page">
        ...
        <div className="detail-grid">
          <div className="stack">
            ...
            <section className="panel stack">
              ...
              {undatedAssessments.map((item) => <div className="status-box" key={item.id}><span>{item.source === "course_plan" ? "Plano de ensino · Data não informada" : "Cadastro manual · Data não informada"}</span><strong>{item.name}</strong><p>{weightSummary(item)}</p><p>{topicsToText(item.topics) || "Conteúdo não informado"}</p><div className="button-row"><button type="button" onClick={() => { setSelectedAssessment(item); setAssessmentAction("edit"); }}>Definir data</button><button className="secondary-button" type="button" onClick={() => { setSelectedAssessment(item); setAssessmentAction("edit"); }}>Editar</button><button className="secondary-button" type="button" onClick={async () => { await updateAssessment(disciplineId, item.id, { status: "cancelled" }); await refreshRecords("Avaliação cancelada."); }}>Cancelar</button><button className="secondary-button" type="button" onClick={async () => { await deleteAssessment(disciplineId, item.id); await refreshRecords("Avaliação excluída."); }}>Excluir</button></div></div>)}
            </section>
            </section>
            <section className="panel stack"><div className="section-heading"><div><h2>Avaliações realizadas</h2><p>Somente estas, com nota, entram na média parcial.</p></div><button type="button" onClick={() => { setSelectedAssessment(null); setAssessmentAction("completed"); }}>Registrar realizada</button></div>{completedAssessments.length === 0 && <p className="message muted">Nenhuma avaliação realizada com nota.</p>}{completedAssessments.map((item) => <div className="status-box" key={item.id}><span>{item.date ?? "sem data"}</span><strong>{item.name} · Nota: {numberText(item.grade)}</strong>{item.group_final_weight != null && item.group_weight != null ? <p>Grupo vale {numberText(item.group_final_weight, "%")} da disciplina · {numberText(item.group_weight, "%")} dentro do grupo · peso efetivo na nota final: {numberText(effectiveWeight(item), "%")}</p> : <p>Peso efetivo na nota final: {numberText(effectiveWeight(item), "%")}</p>}<p>{topicsToText(item.topics) || "Conteúdo não informado"}</p><div className="button-row"><button className="secondary-button" type="button" onClick={() => { setSelectedAssessment(item); setAssessmentAction("edit"); }}>Editar</button><button className="secondary-button" type="button" onClick={async () => { await deleteAssessment(disciplineId, item.id); await refreshRecords("Avaliação excluída."); }}>Excluir</button></div></div>)}</section>
          </div>
          <div className="stack">
      ...
      {activeTab === "coursePlan" && (
        <div className="detail-grid">
          <section className="panel stack"><h2>Plano de ensino</h2><p>O PDF é usado só para extração temporária. A persistência ocorre apenas após confirmação dos dados estruturados.</p><label>Enviar PDF<input type="file" accept="application/pdf" onChange={(event) => void handlePlanPreview(event.target.files?.[0])} /></label>{preview?.warnings.map((warning) => <p className="message warning" key={warning}>{warning}</p>)}{previewData ? <><div className="status-box"><span>Resumo da extração</span><strong>{previewData.evaluation_groups.length} grupo(s) de avaliação · {previewData.assessments.length} componente(s)</strong><p>{previewData.assessments.filter((item) => !item.date).length} componentes ainda sem data · Fonte: {preview?.source === "gemini" ? "Gemini" : "parser local"}</p>{coursePlanPreviewState(preview, previewData) && <p>{coursePlanPreviewState(preview, previewData)}</p>}</div><details className="simulation-details"><summary>Revisar itens extraídos</summary><CoursePlanEditor data={previewData} onChange={setPreviewData} /></details><div className="form-actions"><button className="secondary-button" type="button" onClick={() => { setPreview(null); setPreviewData(null); }}>Descartar prévia</button><button type="button" disabled={saving || previewData.assessments.some((item) => item.status === "requires_review")} onClick={handlePlanConfirm}>Confirmar dados estruturados</button></div></> : coursePlan ? <div className="stack"><div className="status-box"><span>Estado</span><strong>Plano confirmado</strong></div><h3>Conteúdos</h3><ul>{coursePlan.contents.map((item) => <li key={item}>{item}</li>)}</ul><h3>Cronograma</h3><ul>{coursePlan.schedule.map((item) => <li key={item}>{item}</li>)}</ul><h3>Avaliações extraídas</h3><div className="status-box"><span>Resumo</span><strong>{coursePlan.evaluation_groups.length} grupo(s) · {coursePlan.assessments.length} componente(s)</strong><p>{coursePlan.assessments.filter((item) => !item.date).length} componentes ainda sem data</p></div><details className="simulation-details"><summary>Revisar itens</summary><ul>{coursePlan.assessments.map((item) => <li key={item.name}>{item.name} · {item.date ?? "Data não informada"} · {weightSummary(item)}</li>)}</ul></details><button className="secondary-button" type="button" onClick={async () => { await deleteCoursePlan(disciplineId); await refreshRecords("Plano removido."); }}>Remover dados estruturados</button></div> : <p className="message muted">Nenhum plano confirmado. Envie um PDF para pré-visualizar.</p>}</section>
